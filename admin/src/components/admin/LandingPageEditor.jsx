...
import {
  ...
  Megaphone, Tag, Gamepad2, FileText,
    ...
    <div className="w-full h-full overflow-hidden rounded-2xl border border-slate-700/50 bg-slate-950 shadow-2xl">
      ...
      <div className="overflow-y-auto h-[calc(100%-32px)]" style={{ color: colors.text || '#f1f5f9' }}>
        ...
        {data.showAnnouncements && data.announcements?.length > 0 && (
          <div className="px-2.5 py-1.5" style={{ backgroundColor: (colors.warning || '#f59e0b') + '15' }}>
            {data.announcements.slice(0, 2).map(a => (
              <p key={a.id} className="text-[7px] text-amber-400 truncate">📢 {a.text}</p>
            ))}
          </div>
        )}
