...
// ── Color Maps ───────────────────────────────────────────────
const GRADIENT_MAP = {
  emerald: { from: 'from-emerald-500/20', to: 'to-emerald-600/5', border: 'border-emerald-500/25', iconBg: 'bg-emerald-500/15', icon: 'text-emerald-400', glow: 'shadow-emerald-500/10', badge: 'bg-emerald-500' },
  blue: { from: 'from-blue-500/20', to: 'to-blue-600/5', border: 'border-blue-500/25', iconBg: 'bg-blue-500/15', icon: 'text-blue-400', glow: 'shadow-blue-500/10', badge: 'bg-blue-500' },
  purple: { from: 'from-purple-500/20', to: 'to-purple-600/5', border: 'border-purple-500/25', iconBg: 'bg-purple-500/15', icon: 'text-purple-400', glow: 'shadow-purple-500/10', badge: 'bg-purple-500' },
  amber: { from: 'from-amber-500/20', to: 'to-amber-600/5', border: 'border-amber-500/25', iconBg: 'bg-amber-500/15', icon: 'text-amber-400', glow: 'shadow-amber-500/10', badge: 'bg-amber-500' },
  pink: { from: 'from-pink-500/20', to: 'to-pink-600/5', border: 'border-pink-500/25', iconBg: 'bg-pink-500/15', icon: 'text-pink-400', glow: 'shadow-pink-500/10', badge: 'bg-pink-500' },
  cyan: { from: 'from-cyan-500/20', to: 'to-cyan-600/5', border: 'border-cyan-500/25', iconBg: 'bg-cyan-500/15', icon: 'text-cyan-400', glow: 'shadow-cyan-500/10', badge: 'bg-cyan-500' },
  violet: { from: 'from-violet-500/20', to: 'to-violet-600/5', border: 'border-violet-500/25', iconBg: 'bg-violet-500/15', icon: 'text-violet-400', glow: 'shadow-violet-500/10', badge: 'bg-violet-500' },
  indigo: { from: 'from-indigo-500/20', to: 'to-indigo-600/5', border: 'border-indigo-500/25', iconBg: 'bg-indigo-500/15', icon: 'text-indigo-400', glow: 'shadow-indigo-500/10', badge: 'bg-indigo-500' },
}

// ── Live Badge Component ─────────────────────────────────────
function LiveBadge({ count, label, color }) {
  const colors = GRADIENT_MAP[color] || GRADIENT_MAP.emerald
  ...
  return (
    <motion.div
      ...
      style={{ backgroundColor: color === 'emerald' ? '#10b981' : color === 'blue' ? '#3b82f6' : color === 'purple' ? '#a855f7' : color === 'amber' ? '#f59e0b' : color === 'pink' ? '#ec4899' : color === 'cyan' ? '#06b6d4' : color === 'violet' ? '#8b5cf6' : '#6366f1' }}
    >
...
function AdminCard({ card, index, liveCount }) {
  ...
  const formatPrimary = () => {
    ...
    if (typeof primary === 'object' && primary instanceof Date) {
      return primary.toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })
    }
    if (typeof primary === 'number') {
      if (primary >= 100000) return `₹${(primary / 100000).toFixed(1)}L`
      return primary.toLocaleString('en-IN')
    }
    return '—'
  }
  ...
  return (
    ...
    >
      {/* Glass Card */}
      <div className={`relative bg-gradient-to-br ${colors.from} ${colors.to} backdrop-blur-xl border ${colors.border} rounded-2xl p-5 transition-all duration-300 hover:shadow-xl hover:${colors.glow}`}>
        ...
        <div className="relative z-10">
            ...
            <div className="flex items-center gap-2">
              <AnimatePresence mode="wait">
                {liveCount !== null && liveCount !== undefined && (
                  ...
                  >
                    <LiveBadge count={liveCount} color={card.color} />
                  </motion.div>
                )}
              </AnimatePresence>
              {isRefetching && (
          ...
          <div className="flex items-center justify-between">
            <span className="text-lg font-bold text-white">{formatPrimary()}</span>
            <div className="flex items-center gap-1">
              <Eye className="w-3.5 h-3.5 text-slate-500 opacity-0 group-hover:opacity-100 transition-opacity" />
              <Zap className="w-3 h-3 text-slate-600 opacity-0 group-hover:opacity-100 group-hover:text-amber-400 transition-all" />
            </div>
          </div>
        </div>
...
export default function AdminDashboardOverview() {
  ...
  // Generate badge text from live counts
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-400 mt-1">Real-time platform metrics — live updates via Supabase</p>
      </div>
